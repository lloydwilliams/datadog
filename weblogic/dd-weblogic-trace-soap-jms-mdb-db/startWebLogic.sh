...
DOMAIN_HOME="/Users/lloyd.williams/u01/weblogic/wls12214/user_projects/domains/base_domain"
...
if [ "${WLS_REDIRECT_LOG}" = "" ] ; then
	...
	echo "${JAVA_HOME}/bin/java -javaagent:/Users/lloyd.williams/u01/datadog/dd-java-agent.jar -Ddd.trace.config=/Users/lloyd.williams/u01/weblogic/dd-config.properties -Ddd.profiling.enabled=true -XX:+UnlockCommercialFeatures -XX:+FlightRecorder -XX:FlightRecorderOptions=stackdepth=256 ${JAVA_VM} ${MEM_ARGS} ${LAUNCH_ARGS} -Dweblogic.Name=${SERVER_NAME} -Djava.security.policy=${WLS_POLICY_FILE} ${JAVA_OPTIONS} ${PROXY_SETTINGS} ${SERVER_CLASS}"
	${JAVA_HOME}/bin/java -javaagent:/Users/lloyd.williams/u01/datadog/dd-java-agent.jar -Ddd.trace.config=/Users/lloyd.williams/u01/weblogic/dd-config.properties -Ddd.profiling.enabled=true -XX:+UnlockCommercialFeatures -XX:+FlightRecorder -XX:FlightRecorderOptions=stackdepth=256 ${JAVA_VM} ${MEM_ARGS} ${LAUNCH_ARGS} -Dweblogic.Name=${SERVER_NAME} -Djava.security.policy=${WLS_POLICY_FILE} ${JAVA_OPTIONS} ${PROXY_SETTINGS} ${SERVER_CLASS}
else
